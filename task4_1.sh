...
man=`dmidecode -s baseboard-manufacturer`
if [[ -z $man ]]; then man=Unknown; fi
prod=`dmidecode -s baseboard-product-name`
...
echo Installation date: `tune2fs -l /dev/sda1 | grep 'Filesystem created:' | awk '{$1=$2=""; print $0}' | sed -e 's/^[ \t]*//'` >> $FILE_NAME
